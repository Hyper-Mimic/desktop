...
const generateMetainfo = (releases) => {
  ...
  for (const {version, date, notes} of releases) {
    ...
    xml += `      <url type="details">https://github.com/Hyper-Mimic/desktop/releases/tag/v${version}</url>\n`;
    xml += '      <description>\n';
    xml += '        <ul>\n';
  ...
  const path = pathUtil.join(import.meta.dirname, '../linux-files/org.hypermimic.HyperMimic.metainfo.xml');
  let source = fs.readFileSync(path, 'utf-8');
  source = source.replace(
