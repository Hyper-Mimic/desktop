...
If you're looking for downloads, head to: https://hypermimic.netlify.app/desktop
...
The website source code is in the `desktop` folder of the `Hyper-Mimic/hypermimic.netlify.app` repository.
...
Due to the security requirements mandated by custom extensions existing, our desktop app is significantly more complicated than Scratch's.
 ...
 - **dist-library-files** and **dist-extensions** contain additional static resources managed by `yarn run fetch`
...
We've found that development can work pretty well if you open two terminals side-by-side and run `yarn run webpack:watch` in one and `yarn run electron:start` in the other. You can refresh the windows with ctrl+R or cmd+R for renderer file changes to apply, and manually restart the app for main file changes to apply.

## Linux sandbox helper error

On some Linux distributions, Electron will crash with the message `The SUID sandbox helper binary was found, but is not configured correctly. Rather than run without sandboxing I'm aborting now. You need to make sure that /home/.../hypermimic-desktop/node_modules/electron/dist/chrome-sandbox is owned by root and has mode 4755.`. Notably we have seen this happen on Debian 10 and earlier and Ubuntu 24.04 and later.
 ...
 * Privacy policy: https://hypermimic.netlify.app/desktop/privacy.html

## Advanced customizations

HyperMimic Desktop lets you configure custom JS and CSS without rebuilding the app.

Find HyperMimic Desktop's data path by using the list below or by clicking "?" in the top right corner, then "Desktop Settings", then "Open User Data", then opening the highlighted folder, or refer to this list:

 - Windows (except Microsoft Store): `%APPDATA%/hypermimic-desktop`
 - Microsoft Store: Open `%LOCALAPPDATA%/Packages`, find the folder with the word `HyperMimicDesktop` in it, then open `LocalCache/Roaming/hypermimic-desktop`
 - macOS (except Mac App Store): `~/Library/Application Support/hypermimic-desktop`
 - Mac App Store: `~/Library/Containers/org.hypermimic.desktop/Data/Library/Application Support/hypermimic-desktop` (note that the `org.hypermimic.desktop` part may appear as `HyperMimic` in Finder)
 - Linux (except Flatpak and Snap): `~/.config/hypermimic-desktop`
...
Create the file `userscript.js` in this folder to configure custom JS. Create the file `userstyle.css` in this folder to configure custom CSS. Completely restart HyperMimic Desktop (including all windows) to apply.

## Uninstall

See https://hypermimic.netlify.app/desktop/uninstall
